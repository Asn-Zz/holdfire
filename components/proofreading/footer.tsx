...
const INFOS: FooterInfo[] = [
    ...
    {
        title: '错误分类',
        description: '错误分类清晰，可快速定位错误位置',
        icon: Library,
        color: 'green'
    },
    {
        title: '安全可靠',
        description: '全代码开源，本地运行，不上传数据',
        icon: LockKeyhole,
        color: 'red'
    },
    {
        title: '本地存储',
        description: '全量数据本地存储，可查看历史记录',
        icon: Database,
        color: 'indigo'
    }
]
...
export function Footer() {
    return (
        <Card>
            ...
            <CardContent>
                <p className="text-gray-700 mb-4">文章校对助手是一款专为内容创作者设计的智能校对工具，能够帮助您快速校对文章。</p>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                    {INFOS.map((item) => (
                        <div key={item.title} className={`bg-${item.color}-50 p-4 rounded-lg`}>
                            <div className="flex items-center mb-3">
                                <div className={`w-10 h-10 rounded-full bg-${item.color}-100 flex items-center justify-center text-${item.color}-600`}>
